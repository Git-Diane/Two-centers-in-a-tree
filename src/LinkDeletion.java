...
public class LinkDeletion { // PROPOSED SOLUTION NUMBER 2

	
	// Which test file we are using
    public static int fileNum = 9;
	...
	public static int still_in_tree; // a variable in Goldman's algorithm ; see class "Goldman" for more information
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(J);
		//LinkDeletion.writeOutput(runTime); // we used this line of code to write the output files

	}
	
	public static void writeOutput(long runTime) { //writes the output in text files
		try {
            File statText = new File("C:/Users/Diane/eclipse-workspace/INF 421 project/outputLinkDeletion/output."+fileNum+".txt");
	...
	public static void cut(int v1, int v2) { // Computes J(T1,T2) where T1 is the subtree "starting from" v1 and T2 the subtree "starting from v1's neighbor v2
	...
	public static long J(int newc) {// uses BFS to compute S_i(c_i) (i =1 or 2) 
		long j = 0;
	...
	public static int goldman(int v,int W) { // runs goldman's algorithm to find the 1-center of a subtree
		// v : starting point of Goldman's algorithm
		still_in_tree = v;
		stop = false;
		while (!stop) {
			stop = step(W);
	...
	public static void back_to_zero() { // the weights and adjacency lists have been modified by goldman's algorithm, we have to re-initialize them to their original values
		weights = tree.w.clone();
		adj_lists = tree.adj_lists();
	}
}
